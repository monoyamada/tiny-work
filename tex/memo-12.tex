...
\subsection{思考実験}\label{s2:思考実験} %{
	...
	\begin{todo}[関数とは]\label{todo:関数とは} %{
		関数とは次のメソッドを持ったオブジェクトとしてもいいのでは？
		\begin{description}\setlength{\itemsep}{-1mm} %{
			\item[入出力] 入力変数と出力変数それぞれの数と型
			\item[apply] $\beta$変換
			\item[currying] シンタックスシュガーとしていいし、ビルトイン関数
			としてもいい。
			\item[composite] シンタックスシュガーとしていいし、ビルトイン関数
			としてもいい。
		\end{description} %}
	\end{todo} %todo:関数とは}
%s2:思考実験}
