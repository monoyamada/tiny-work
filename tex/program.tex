...
Furthermore, the map $\iota$:
\begin{equation}\begin{split}
	\iota: \mybf{N} &\to BX \\
		k &\mapsto \iota k \text{ such that} 
			(\iota k)x = k \text{ for all }x\in X \\
\end{split}\end{equation}
is a ring-homeo.
$0_B$ and $1_B$ are given by $\iota 0$ and $\iota 1$, respectively.
Therefore $BX$ can be view as semi-module over $\myop{N}$ by the following 
identification:
\begin{equation}\begin{split}
	k.m_Bf := (\iota k).m_Bf \text{ for all }k\in \myop{N} \text{ and }f\in BX
\end{split}\end{equation}
...
of $X$ in the $BX$. Let the map $\kappa$ is the followings:
\begin{equation}\begin{split}
	\kappa: X &\to BX \\
		x &\mapsto \kappa x \text{ such that }(\kappa x)y = \begin{cases}
			1 & \text{iff } x = y \\
			0 & \text{otherwise} \\
			\end{cases} \text{ for all } y\in X \\
\end{split}\end{equation}
. $BX$ is spanned by $\kappa X$ as the followings:
\begin{equation}\begin{split}
	f = \sum_{x\in X}(fx).m_B(\kappa x) \text{ for all }f\in BX \\
\end{split}\end{equation}
. Where summation is taken by $+_B$.
Any element of the set $\set{X^{\times2}\to \mybf{N}^{\times2}}$
is written by $\kappa X$ in the same way. The following equation is satisfied
for all $f\in \set{X^{\times2}\to \mybf{N}^{\times2}}$:
\begin{equation}\begin{split}
	f &= \sum_{(x,y)\in X^{\times2}}\kakko{f\kakko{x,y}}.m_B\kakko{\kakko{\kappa x}\otimes\kakko{\kappa y}}
\end{split}\end{equation}
.

\begin{proposition}[BX is everything]
Let $X$ is a finite set, $BX$ is a bag of $X$.
Let $BX=(BX,+_B,0_B,m_B,1_B)$ is a semi-algebra over $\myop{N}$
incuced by the ring structure of $\myop{N}$.
The following isomorphism is satisfied:
\begin{equation}\begin{split}
	BX\otimes BX \simeq \set{X^{\times2}\to \mybf{N}^{\times2}} \\
\end{split}\end{equation}
.
\end{proposition}

\begin{proposition}[A set of symmetrized words is a bag] 
Let $X$ be a finite set, $FX=(FX,*,1_F)$ be a free-monoid of $X$
, $BX$ be a bag of $X$. Let $\sim$ be the following relation:
\begin{equation}\begin{split}
	w_1.m_Fw_2 \sim w_2.m_Fw_1 \text{ for all }w_1,w_2\in FX
\end{split}\end{equation}
\end{proposition}
