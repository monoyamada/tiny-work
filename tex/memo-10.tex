...
\subsection{シャッフル積}\label{s2:シャッフル積} %{
	...
	\begin{definition}[置換の大きさ]\label{def:置換の大きさ} %{
		$S_n$を$n$次対称群とする。
		任意の$\sigma\in S_n$に対して、$\sigma$をあみだくじとして書いた時に
		必要となる\underline{最小の}横線の数を$|\sigma|$と書く。
	\end{definition} %def:置換の大きさ}

	$3$次対称群の場合、巡回置換を用いるとその大きさは次のようになる。
	\begin{equation*}\begin{split}
		|()| = 0,\quad |(1,2)| = |(2,3)| = |(3,1)| = 1
		,\quad |(1,2,3)| = |(1,3,2)| = 2
	\end{split}\end{equation*}
	一般に置換の大きさは次の三角不等式を満たす。
	\begin{equation*}\begin{split}
	...
	置換の大きさを用いると、q-シャッフル積は次のように書くことができる。
	\begin{equation*}\begin{split}
