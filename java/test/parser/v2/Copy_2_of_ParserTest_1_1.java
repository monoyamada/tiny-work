...
public class Copy_2_of_ParserTest_1_1 extends ParserTest_1_0 {
	...
	static class FlattenEquation extends Equation {
		...
		static FlattenEquation[] build(TreeAnalyzer analyzer)
				throws ParserException {
			AdjacencyEquation[] ads = analyzer.adjacencyEquations();
			FlattenEquation[] out = new FlattenEquation[ads.length];
			for (int i = 0, n = ads.length; i < n; ++i) {
				Variable[][] eq = build(ads[i]);
				out[i] = new FlattenEquation(ads[i].variable(), eq);
			}
			return out;
		}
		private static Variable[][] build(AdjacencyEquation eq) {
			// Variable x = eq.variable();
			AdjacencyNode[] follows = eq.firstNodes;
			List<Variable[]> buffer = new ArrayList<Variable[]>();
			for (int i = 0, n = follows.length; i < n; ++i) {
				List<Variable> pre = new ArrayList<Variable>();
				build(buffer, pre, follows[i]);
			}
			int n = buffer.size();
			Variable[][] out = new Variable[n][];
			return buffer.toArray(out);
		}
		private static Collection<Variable[]> build(Collection<Variable[]> output,
				List<Variable> succ, AdjacencyNode node) {
			Variable x = node.variable();
			List<AdjacencyNode> follows = node.follows();
			if (follows.size() < 1) {
				switch (x.type()) {
				case Variable.ZERO:
				break;
				case Variable.ONE:
					if (succ.isEmpty()) {
						...
						output.add(succ.toArray(Variable.EMPTY_ARRAY));
					}
				break;
				default:
					succ.add(x);
			...
			succ.add(x);
			for (int i = 0, n = follows.size(); i < n; ++i) {
				build(output, succ, follows.get(i));
			}
			succ.remove(succ.size() - 1);
			return output;
		}

		final Variable[][] equations;

		FlattenEquation(Variable variable, Variable[][] equations) {
			super(variable);
		...
		Appendable writeInfix(Appendable output) throws IOException {
			...
			for (int i = 0, n = terms.length; i < n; ++i) {
				...
				Variable[] term = terms[i];
				for (int ii = 0, nn = term.length; ii < nn; ++ii) {
					if (ii != 0) {
						output.append(' ');
					}
					output.append(term[ii].name());
				}
			}
			return output;
		}
