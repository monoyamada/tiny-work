...
/**
 * This is a simple implementation of combinadics in Java
 * Call the getMaxNumberOfCombinations ( slots, tokens ) method to essentially
 * get C ( slots, tokens )
 * Call the pascalsTriangle ( rows ) to get a Pascal triangle up to 'rows' row
 * Call the printCombination ( slots, tokens, index ) method to print out the
 * index'th combination for C ( slots, tokens )
 * Call the printAllCombinations ( slots, tokens ) method to print out all the
 * combinations for C ( slots, tokenss )
...
public class Combo
{
	...
	public static void main(String[] args)
	{
		...
		combo.printAllCombinations(8, 5);
	}
	/**
	 * Returns the maximum number of combinations for C(slots,tokens)
	 * This is using a recursive algorithm so as to guarantee that no number in
	 * the calculation of the answer will go over MAXLONG
	 * as long as the answer itself also does not go over MAXLONG
	 * Normally, one would calculate full or partial factorials to get this answer
	 * per the equation: ( slots!/((slots-tokens)!*tokens!) )
	 * However, intermediate steps in that calculation may involve numbers bigger
	 * than MAXLONG even if the answer itself is not bigger than MAXLONG
	 * @param slots -
	 *          the possible values (from 1) in a combination
	 * @param tokens -
	 *          the number of values in a combination
	...
	public long getMaxNumberOfCombinations(long slots, long tokens)
	{
		...
		return getMaxNumberOfCombinations(slots - 1, tokens - 1)
				+ getMaxNumberOfCombinations(slots - 1, tokens);
	}
	/**
	 * Print out Pascal's triangle up to 'rows' row
	 * @param rows -
	 *          number of rows to print out
	 */
	 ...
	 *          the number of values in a combination
	 * @param index -
	 *          unique identifier declaring which combination to print out. Should
	 *          be between 0 and C(slots, tokens)
	 */
